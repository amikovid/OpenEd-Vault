...
def _skill_root() -> Path:
    """Get the seomachine skill root (.claude/skills/seomachine)"""
    return Path(__file__).resolve().parent.parent


def _vault_root() -> Path:
    """Get the OpenEd Vault root (where .env lives)"""
    skill_root = _skill_root()
    # .claude/skills/seomachine -> vault root (3 levels up)
...
def _import_modules():
    # Import from sibling modules folder
    modules_path = _skill_root() / "modules"
