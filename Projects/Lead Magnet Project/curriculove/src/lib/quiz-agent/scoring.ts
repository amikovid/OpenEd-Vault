...
const FREE_TEXT_SIGNALS: FreeTextSignal[] = [
  { pattern: /\b(classical|great books|trivium|latin|socratic)\b/i, deltas: { CL: 10 } },
  { pattern: /\b(charlotte mason|living books|narration|copywork)\b/i, deltas: { CM: 10 } },
  { pattern: /\b(traditional|textbook|worksheets?|grades?|tests?|school[- ]at[- ]home)\b/i, deltas: { TR: 8 } },
  { pattern: /\b(montessori|prepared environment|practical life)\b/i, deltas: { MO: 10 } },
  { pattern: /\b(waldorf|rhythm|imagination|handwork)\b/i, deltas: { WA: 10 } },
  { pattern: /\b(unschool(ing)?|child[- ]led|self[- ]directed|no curriculum)\b/i, deltas: { UN: 10 } },
  { pattern: /\b(wild\s*\+?\s*free|wonder[- ]based|beauty|community)\b/i, deltas: { WF: 8 } },
  { pattern: /\b(nature[- ]based|forest school|outdoor[s]?|nature study)\b/i, deltas: { NB: 8 } },
  { pattern: /\b(project[- ]based|real[- ]world|problem[- ]solving)\b/i, deltas: { PB: 8 } },
  { pattern: /\b(eclectic|mix and match|pick and choose|flexible mix)\b/i, deltas: { EC: 8 } },
  { pattern: /\b(microschool|learning pod|co[- ]?op|cooperative)\b/i, deltas: { MS: 8 } },
  { pattern: /\b(faith[- ]based|christian|biblical|scripture|god[- ]centered)\b/i, deltas: { FB: 12 } },
];
  ...
  const candidates = QUESTION_BANK.filter(
    (q) =>
      ...
      q.targetPairs.some((pair) => {
        ...
        if (pair.endsWith("-confirm")) {
          const tag = pair.split("-")[0] as PhilosophyTag;
          return top?.tag === tag && top.score >= 35;
        }
        const normalized = normalizePair(pair);
        return normalized ? normalizedAmbiguous.has(normalized) : false;
      })
  );
...
/**
 * Score free-text notes using lightweight keyword signals.
 */
export function scoreFreeText(text: string): ScoreDeltas {
  const deltas: ScoreDeltas = {};
  const normalized = text.trim();
  if (!normalized) return deltas;

  for (const signal of FREE_TEXT_SIGNALS) {
    if (signal.pattern.test(normalized)) {
      for (const [tag, delta] of Object.entries(signal.deltas)) {
        const key = tag as PhilosophyTag;
        deltas[key] = (deltas[key] || 0) + (delta || 0);
      }
    }
  }
  return deltas;
...
function normalizePair(pair: string): string | null {
  ...
  if (!a || !b) return null;
  if (!ALL_PHILOSOPHIES.includes(a as PhilosophyTag)) return null;
  if (!ALL_PHILOSOPHIES.includes(b as PhilosophyTag)) return null;
