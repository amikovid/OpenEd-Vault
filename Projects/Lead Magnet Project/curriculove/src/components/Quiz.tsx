...
export default function Quiz({ onComplete }: QuizProps) {
  ...
  const submitAnswer = async (optionIndex: number) => {
    ...
    try {
      ...
      const trimmedNote = freeText.trim();
        ...
        <div className="mt-6">
          {!showNote ? (
            ...
            <div className="space-y-2">
              <label className="text-xs text-[var(--foreground-secondary)]" htmlFor="quiz-note">
                Anything else about your homeschool style?
              </label>
              <textarea
                id="quiz-note"
                value={freeText}
                onChange={(e) => setFreeText(e.target.value)}
                placeholder="e.g., short lessons, lots of outdoors, faith-centered..."
                className="w-full h-24 p-3 border border-[var(--border)] rounded-xl bg-[var(--surface)] text-[var(--foreground)] resize-none focus:outline-none focus:ring-2 focus:ring-[var(--primary)]"
                disabled={state.isLoading}
              />
              <button
                type="button"
                ...
                className="text-xs text-[var(--foreground-muted)] underline"
              >
                Remove note
              </button>
            </div>
