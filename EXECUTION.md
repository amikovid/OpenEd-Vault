# EXECUTION.md

**What's happening this week** - Single source of truth for active work.

**Last Updated:** 2026-01-29

---

## This Week's Focus (Jan 27 - Feb 2)

### Priority 1: Q1 Launch Prep

| Task | Owner | Due | Status |
|------|-------|-----|--------|
| Show Melissa Search Console integration | Charlie | Jan 30 | ⏳ |
| Talk to Melissa about daily cadence | Charlie | Jan 30 | ⏳ |
| Set up Monday SEO report to Slack | Charlie | Feb 3 | ⏳ |
| Request Databox access from Alex | Charlie | This week | ⏳ |

### Priority 2: Project Dandelion Sprint 01

| Task | Owner | Due | Status |
|------|-------|-----|--------|
| Meta retargeting campaign setup | Elijah | Feb 7 | 🔄 In Progress |
| Short-form video tests | Elijah | Feb 7 | 🔄 In Progress |
| YouTube ads research | Elijah | Feb 7 | ⏳ |

### Priority 3: Guest Contributor Outreach

| Contact | Action | Status |
|---------|--------|--------|
| Justin Skycak | Send pitch email | 🔥 HOT LEAD |
| Mason Pashia | Follow up | ⏳ |
| Janssen Bradshaw | Coordinate SEO topic | ✅ AGREED |
| Savvy Learning | Coordinate with Ela | ⏳ |

---

## Active Projects (Full List)

### Projects Folder

| Project | Status | Next Action | Due |
|---------|--------|-------------|-----|
| **Project Dandelion** | 🔄 Active | Sprint 01 Week 2 - ads live | Feb 7 |
| ↳ Retargeting Strategy | 🔄 Active | Build Pillar 1 ads (child of Dandelion) | Feb 9-15 |
| **Eddie Awards** | 📋 Planning | Finalize branding with Ella | Feb 10 |
| **Lead Magnet / Curriculove** | 🔄 Active | Quiz dev continues | Ongoing |
| **Tools Directory** | 🔄 Active | Publish Teaching Textbooks | This week |
| **KPI Discussions** | 📋 Planning | Q1 bonus structure | TBD |
| **Content Staging Pipeline** | 📦 Archived | Absorbed into Social Media/PROJECT.md | — |

### Studio (Ongoing Workflows)

| Workflow | Cadence | Status |
|----------|---------|--------|
| OpenEd Daily Newsletter | Mon-Thu | ✅ Active |
| OpenEd Weekly Newsletter | Friday | ✅ Active |
| Podcast Production | Weekly | ✅ Active (2 episodes in progress) |
| SEO Content Production | As needed | ✅ Active |
| Social Media | Daily | ✅ Active |

### Studio (Projects)

| Project | Status | Notes |
|---------|--------|-------|
| Meta Ads | ✅ Ready | 100 concepts done, awaiting Retargeting launch |
| Analytics & Attribution | 🔄 Active | HubSpot setup complete, QBR items pending |
| Nearbound Pipeline | 🔄 Active | 81 profiles, social enrichment ongoing |
| Content Staging Pipeline | 📦 Archived | Merged into Social Media/PROJECT.md (2026-02-04) |

---

## Key Dates

| Date | Event | Impact |
|------|-------|--------|
| **Feb 10** | Enrollment tasks open | Interest → Application conversion focus |
| **Feb 16** | Retargeting Pillar 1 launch | 5 ad variations needed |
| **Feb 17** | Eddy Awards hype begins | Internal pre-launch |
| **March 1** | Pillars 2 & 3 launch | Website visitors + lookalikes |
| **March 8** | Eddy nominees announced | Daily content for 2 weeks |

---

## Blocked Items

| Item | Blocker | Owner |
|------|---------|-------|
| GSC Access | Need ops to add service account | Pending |
| Meta Pixel validation | Needs verification before Pillar 2 | Ella |
| Lead lists for retargeting | Pull from CRM | Ella |

---

## Decisions Needed

- [ ] **Daily newsletter cadence** - 3-4/week vs Mon-Thu (discuss with Melissa)
- [ ] **YouTube ads test** - Go/no-go decision for fall enrollment ($1,500-2,000 test budget)

---

## Completed This Week

- [x] Restored dude-with-sign-writer skill (2026-01-29)
- [x] Created EXECUTION.md (2026-01-29)
- [x] Vault audit complete (2026-01-29)
- [x] Merged duplicate Open Education Hub folders (2026-01-29)
- [x] Deleted empty sync-webflow skill (2026-01-29)
- [x] Moved 4 stray files from Studio root (2026-01-29)
- [x] Deleted test todo file (2026-01-29)
- [x] Reconceived Project Dandelion as paid acquisition hub (2026-01-29)
- [x] Updated PROJECT.md with research findings and cross-references (2026-01-29)
- [x] Linked Retargeting Strategy as child of Dandelion (2026-01-29)
- [x] Created playbook/ folder structure (2026-01-29)
- [x] Updated Meta Ads PROJECT.md with Dandelion cross-references (2026-01-29)
- [x] Archived 67 legacy Ad Creative Concepts files (2026-01-29)
- [x] Deleted redundant Notion CSV export (2026-01-29)
- [x] Moved beehiiv webinar PDF to Archive (2026-01-29)
- [x] Created OpenEd Content OS project (2026-01-29)
- [x] Mapped all skill chains: newsletter, podcast, SEO, video (2026-01-29)
- [x] Defined auto-invoke triggers for skill chaining (2026-01-29)
- [x] Documented format inventory (360+ templates, 20+ video formats) (2026-01-29)
- [x] Defined repurposing agent concept (2026-01-29)
- [x] Moved Retargeting Strategy INTO Project Dandelion as subfolder (2026-01-29)
- [x] Archived _content-engine-refactor folder to Projects/_archive/ (2026-01-29)
- [x] Moved sub-agent prompts to content-repurposer skill references (2026-01-29)
- [x] Updated all nearbound path references across skills (2026-01-29)
- [x] Consolidated Notion staging docs: merged Notion Social Staging → Content Staging Pipeline (2026-01-29)
- [x] Moved Q3 planning docs into KPI Discussions (2026-01-29)
- [x] Moved Mason Pashia doc to Guest Contributors (2026-01-29)
- [x] Updated SKILL_ARCHITECTURE_MAP.md with current paths (2026-01-29)
- [x] Cleaned Content OS PROJECT.md - removed verbose session log (2026-01-29)

---

## Notes

**Nano folder:** Pending skill for image generation prompts - experimental workspace, leave as-is.

**Social Screenshots folder:** Nearly empty (just one dated subfolder). Consider archiving or documenting purpose.

**Skill updates still needed:**
- Link dude-with-sign-writer to meta-ads-creative (add cross-reference)

**Low-priority cleanup:**
- Add .DS_Store to .gitignore (5 files across vault)

---

*Sync to Apple Reminders with 1-3-9 tags. Update at session end.*
